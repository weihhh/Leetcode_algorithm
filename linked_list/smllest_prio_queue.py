...
class least_prio_queue(object):
    ...
    def max_heapify(self,data,i,key=lambda x:x):
        ...
        if smallest!=i:
            ...
            self.max_heapify(data,smallest,key)
    ...
    def insert(self,element):
        ...
        data[self.heapsize-1]=element
        i=self.heapsize
        while i>1 and self.key(data[i//2-1])>self.key(data[i-1]):#因为是减小，所以往上交换即可，下面肯定还是满足堆特性
            data[i//2-1],data[i-1]=data[i-1],data[i//2-1]
            i=i//2

data=[4,17,4,74,3,5,9]
test=least_prio_queue(data)
# print(test.data)
test.insert(7)
# print(test.heap_extract_min(),test.data)
test.show_heap()
