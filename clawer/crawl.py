...
import argparse#替代optparser的命令行参数获取库
import asyncio
import logging
import sys
...
def main():
    ...
    logging.basicConfig(level=levels[min(args.level, len(levels)-1)])#min不会让level值超出合法大小
    ...
    if args.iocp:
        ...
        loop = asyncio.get_event_loop()#三种不同事件循环的模式
